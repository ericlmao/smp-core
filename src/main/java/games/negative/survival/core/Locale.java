...
import games.negative.survival.SMPPlugin;
...
public enum Locale {
    ...
    public static void init(@NotNull SMPPlugin plugin) {
    ...
    private static void saveFile(@NotNull SMPPlugin plugin, @NotNull File file, @NotNull FileConfiguration config) {
        try {
            config.save(file);
        } catch (IOException e) {
    ...
    private static void validateFile(@NotNull SMPPlugin plugin, @NotNull File file) {
        if (!file.exists()) {
